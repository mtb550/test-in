...
|---|---|---|---|
| **[The tree panel](treePanel/main.md)** | The tree on the left. Test projects, test sets, test runs, and everything done to them | 27 | 101 |
| **[The editor panel](editorPanel/main.md)** | Writing test cases, and running a test run. Both editors, and [light mode](editorPanel/lightMode.md) | 47 | 216 |
| **[The view panel](viewPanel/main.md)** | The panel on the right. One test case in full, and what a test run recorded about it | 15 | 65 |
| **[The settings page](setting/main.md)** | Everything set once per machine, and where each value is kept | 11 | 42 |
| **[Automation code and the gutter](codegen/main.md)** | The test methods Testin writes, and how they are kept in step with the tree | 20 | 72 |
...
| **[Inside Testin](internal/main.md)** | The parts that belong to no panel: the search, and the one thing that owns every file | 7 | 71 |

**152 use cases and 691 rules**, each one checked against the code it describes.
...
| **[Standing decisions](decisions.md)** | Nine designs that look wrong until you know why, and what each one costs to reverse |
...
| **[Standing decisions](decisions.md)** | Nine decisions made once, with the reason each one looks wrong, so they are not argued again in every review | Written |
