...
Testin has six parts, and one more piece that belongs to none of them. Each gets
a folder, and every thing a tester does in it gets a page of its own: the story,
its rules, its screens, what happens step by step, and every way it can be
refused.

| Document | What it covers | Where it stands |
|---|---|---|
| **[The product](product.md)** | What is true of all of it: who uses Testin, what they work with, every status, and the rules that hold everywhere | A draft, moved from Notion, not re-checked against today's plugin — [#72](https://github.com/mtb550/test-in/issues/72) |
| **[The tree panel](treePanel/main.md)** | The tree on the left. 27 use cases, 88 rules, 16 screens | **Written** |
| **[Inside Testin](internal/main.md)** | The parts that belong to no panel: the search that opens from anywhere, and the indexer that owns every file | The search is written. The indexer is not |
| The editor panel | Writing test cases, and running them. **[Light mode](lightMode.md)** is one of its windows, and is written | Not written, except light mode |
| The view panel | Details, history and open bugs for the selected test case | Not written |
| The settings page | Everything set once per machine | Not written |
| Automation code and the gutter | The test methods Testin writes, and the marks beside them | Not written |
