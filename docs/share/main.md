...
| **Difference 12** | Rule-SHARE-005 — the same | The import bar stops between test cases now, and still says how many were written. The sync bar stops between files, where it used to finish all 2,246 of them before noticing. The report bar cannot be stopped: the four generators are handed the document to write and never the bar, so nothing inside them has anything to ask. An export has nothing long left to stop, because it reads the tree from memory and writes one file at the end. |
