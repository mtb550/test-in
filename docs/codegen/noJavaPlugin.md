...
the Java plugin the menu reads **Navigate to Test Method**. In an IDE without it
the same entry reads *Navigate to Test Method (needs the Java plugin)*, so one entry
