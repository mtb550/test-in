...
- **Rule-EDITOR-PANEL-174** — A verdict no test case carries is not drawn at
  all.
- **Rule-EDITOR-PANEL-175** — The untouched test cases read **Pending** while
  the test run is open, and **Untested** once it is signed off.
- **Rule-EDITOR-PANEL-176** — The figures are worked out from what the test run
  holds now, not from disk.
- **Rule-EDITOR-PANEL-177** — A test run that has measured nothing shows a blank
  clock, not a row of zeros.
- **Rule-EDITOR-PANEL-178** — The three run labels are hidden, not blank, when
