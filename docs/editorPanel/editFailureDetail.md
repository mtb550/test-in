...
- **Rule-EDITOR-PANEL-166** — The verdict is not touched. Only the four fields
  are written.
- **Rule-EDITOR-PANEL-167** — The entry works on exactly one test case, and only
  when its verdict is **Failed**.
- **Rule-EDITOR-PANEL-168** — The message comes after the test run is written,
  so an edit that was dropped never reports itself as saved.
- **Rule-EDITOR-PANEL-169** — The dialog is the same one `F` opens, filled in
  with what is there.
