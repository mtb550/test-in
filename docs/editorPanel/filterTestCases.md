...
- **Rule 91** — Four things can be filtered on: the priority, the group, the
  module and, in a test run, the run status.
- **Rule 92** — The module list is built from the test cases in this test set,
  so it only ever offers modules that exist.
- **Rule 93** — Choosing nothing in a filter means every test case matches it.
- **Rule 94** — Filtering goes back to the first page.
- **Rule 95** — The button says how many filters are on.
