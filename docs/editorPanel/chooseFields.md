...
- **Rule 20** — Ticking a field shows it at once, in whichever view is on
  screen.
- **Rule 21** — The choice is remembered, and is separate for a test set and a
  test run.
- **Rule 22** — Two fields cannot be changed. **Description** is always shown
  and **ID** is never shown.
- **Rule 23** — A burst of ticks costs one redraw, not one for each.
